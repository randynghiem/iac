...
class AzureRMCosmosDBAccount(AzureRMModuleBase):
    ...
    def exec_module(self, **kwargs):
        ...
        dict_camelize(self.parameters, [
                      'consistency_policy', 'default_consistency_level'], True)
        dict_rename(self.parameters, [
                    'geo_rep_locations', 'name'], 'location_name')
        ...
        if self.parameters.pop('enable_aggregation_pipeline', False):
            self.parameters['capabilities'].append(
    ...
    def create_update_databaseaccount(self):
        ...
        '''
        self.log(
            "Creating / Updating the Database Account instance {0}".format(self.name))
        ...
        try:
            ...
            self.fail(
                "Error creating the Database Account instance: {0}".format(str(exc)))
    ...
    def delete_databaseaccount(self):
        ...
        '''
        self.log(
            "Deleting the Database Account instance {0}".format(self.name))
        try:
            ...
            self.fail(
                "Error deleting the Database Account instance: {0}".format(str(e)))
    ...
    def get_databaseaccount(self):
        ...
        '''
        self.log(
            "Checking if the Database Account instance {0} is present".format(self.name))
        found = False
        try:
            ...
            self.log(
                "Database Account instance : {0} found".format(response.name))
        except CloudError as e:
    ...
    if new is None:
        ...
        if not isinstance(old, list) or len(new) != len(old):
            result['compare'] = 'changed [' + \
                path + '] length is different or null'
            return False
        elif len(old) == 0:
            return True
